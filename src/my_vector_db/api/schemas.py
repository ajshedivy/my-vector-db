...
# ============================================================================
# Admin / Persistence Schemas
# ============================================================================


class SaveSnapshotResponse(BaseModel):
    """Response schema for save snapshot operation."""

    message: str = Field(..., description="Success message")
    snapshot_path: str = Field(..., description="Path to snapshot file")
    timestamp: str = Field(..., description="Snapshot timestamp")
    stats: Dict[str, int] = Field(
        ..., description="Statistics (libraries, documents, chunks)"
    )


class RestoreSnapshotResponse(BaseModel):
    """Response schema for restore snapshot operation."""

    message: str = Field(..., description="Success message")
    stats: Dict[str, int] = Field(
        ..., description="Restored counts (libraries, documents, chunks)"
    )


class PersistenceStatusResponse(BaseModel):
    """Response schema for persistence status check."""

    enabled: bool = Field(..., description="Whether persistence is enabled")
    snapshot_exists: bool = Field(..., description="Whether snapshot file exists")
    operations_since_save: int = Field(
        default=0, description="Operations since last save"
    )
    snapshot_info: Optional[Dict[str, Any]] = Field(
        None, description="Snapshot file info"
    )
